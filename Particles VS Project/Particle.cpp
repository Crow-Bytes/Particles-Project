...
Particle::Particle(RenderTarget& target, int numPoints, Vector2i mouseClickPosition) : m_A(2, numPoints)
{
    ...
    m_radiansPerSec = ((float)rand() / (RAND_MAX)) * PI_P;
    ...
    float theta = ((float)rand() / (RAND_MAX)) * (PI_P / 2);
    float dTheta = (2 * PI_P) / (numPoints - 1);
...
void Particle::unitTests()
{
    ...
    double theta = PI_P / 4.0;
    RotationMatrix r(PI_P / 4); 
